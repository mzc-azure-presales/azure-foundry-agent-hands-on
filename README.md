...
각 실습자 PC에 이 저장소를 먼저 내려받습니다. git이 있으면 `git clone`을, git을 쓰지 않으면 ZIP 다운로드를 사용합니다. 두 방법 모두 결과 폴더 구성은 동일합니다.

#### 방법 A. git clone (권장)

```bash
git clone https://github.com/mzc-azure-presales/azure-foundry-agent-hands-on.git
...
ZIP 방식은 git 이력 없이 파일만 받으므로, 이후 변경 사항을 반영하려면 ZIP을 다시 내려받아야 합니다.
