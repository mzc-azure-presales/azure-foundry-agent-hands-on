...
이 저장소에는 `.python-version`(3.11.9)이 포함되어 있어, `uv sync` 또는 `uv run`을 처음 실행할 때 **uv가 Python 3.11.9를 자동으로 내려받아 고정**합니다. 따라서 Python을 별도로 설치할 필요가 없습니다.

> 수동 설치를 선호한다면 [Python 3.11.9 for Windows](https://www.python.org/downloads/windows/)를 받아 설치(설치 시 `Add python.exe to PATH` 체크) 후 `python --version`이 `Python 3.11.9`를 출력하는지 확인하세요. 최신 버전에서도 일부 예제가 동작하지만, 패키지 호환성을 위해 3.11.9를 권장합니다.
...
- 터미널은 VS Code의 통합 터미널 PowerShell을 사용해도 됩니다.
...
생성된 `.venv`는 VS Code Python 확장이 자동으로 감지합니다. 우측 하단 또는 명령 팔레트(`Python: Select Interpreter`)에서 `.venv`가 선택되면, 코드 자동완성(IntelliSense), ▶ 실행 버튼, F5 디버깅이 모두 정상 동작합니다. 인터프리터가 자동으로 잡히지 않으면 `.\.venv\Scripts\python.exe`를 직접 선택하세요.
...
# Foundry Agent Service / project connection examples
...
# Azure AI Search / Knowledge base examples
...
# Microsoft Learn MCPTool example
...
| Foundry Search connection 생성 | Foundry project Connections | `FOUNDRY_AI_SEARCH_CONNECTION_NAME`, `FOUNDRY_AI_SEARCH_QUERY_TYPE` | 8.1 Search tool 연결 |
| Application Insights 생성 | Azure Portal Application Insights 리소스 | `FOUNDRY_APPLICATIONINSIGHTS_CONNECTION_STRING`, `FOUNDRY_APPLICATIONINSIGHTS_AUTO_CREATE` | 2.3 trace, 8장 monitoring/trace |
...
- VS Code에서 이 저장소 폴더를 열고 PowerShell 터미널을 사용합니다.
