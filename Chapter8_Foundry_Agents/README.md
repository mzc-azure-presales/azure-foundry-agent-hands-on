...
## API key 호출과 Project SDK 호출 및 인증 구조 차이 (8.1, 8.2, 8.3 차이점)

1장부터 7장까지의 기본 실습은 `FOUNDRY_OPENAI_ENDPOINT`와 `FOUNDRY_API_KEY`를 사용해 OpenAI-compatible endpoint를 호출합니다. 반면 이 장은 Foundry 프로젝트 리소스에 agent version을 만들고 여러 외부 리소스 도구들을 다루기 때문에 `FOUNDRY_PROJECT_ENDPOINT`를 사용하며, **각 실습 예제(8.1, 8.2, 8.3)별로 보안 및 내부 인증 요구가 크게 다릅니다.**

### 1) 8.1 (RAG search tool 에이전트): API Key 및 Bearer 인증 가능
* **에이전트 자원 제어(생성/삭제):** Entra ID 토큰 기반(`DefaultAzureCredential`) 인증이 사용됩니다.
* **모델 추적 및 답변 실행:** `.env`에 `FOUNDRY_API_KEY`가 설정되어 있으면, API Key(Bearer 토큰)를 사용하여 `openai` 클라이언트로 편리하게 실행 및 추적(별도의 `az login` 없이 응답 테스트)할 수 있습니다.
* **벡터 스토어:** `AzureKeyCredential`을 사용해 `AZURE_SEARCH_API_KEY`로 Azure AI Search에 다이렉트로 접속합니다.

### 2) 8.2 (Knowledge base 에이전트): Microsoft Entra ID(토큰) 로그인 필수
* **OBO(On-Behalf-Of) 인증 제한:** Foundry IQ/지식 베이스(Knowledge base)와 연동된 도구는 에이전트가 호출할 때 기저에서 **On-Behalf-Of (OBO) 인증 흐름**을 강제합니다.
* **API Key 차단:** 에이전트 답변 실행 시 `openai` 클라이언트에 API Key(`api_key`)가 주입되어 초기화되면 아래와 같은 Bad Request 오류를 내보내며 호출이 제한됩니다.
  > `Tools configured with OBO auth are not supported with API key authentication. Please use a different authentication method.`
  * 개발 환경에서 `FOUNDRY_API_KEY`가 선언되어 있더라도, **8.2 코드는 명시적으로 API Key를 배제하고 Entra ID 토큰(`DefaultAzureCredential`) 방식을 통해 클라이언트를 초기화하여 호출해야 합니다.**
* **인프라 파이프라인 (`ensure_kb_mcp_project_connection`):** 지식 베이스 MCP 연동용 Remote Connection을 ARM REST API(`management.azure.com`)로 자동 구축하므로 Microsoft Entra AD 관리자 로그인 토큰이 반드시 동작해야 합니다.

### 3) 8.3 (Microsoft Learn MCP 에이전트): Microsoft Entra ID(토큰) 로그인 필수
* **이유:** 외부/원격 MCP 서비스를 Remote 툴 커넥션으로 에이전트에 바인딩할 때 역시 OBO 인증의 일환으로 연결 매핑이 제어됩니다.
* **인증 해결:** 8.2 장과 동일하게 `api_key`를 OpenAI 클라이언트에 매핑하지 않고, `DefaultAzureCredential` 기반의 Entra ID 토큰만으로 대화 API(`openai.responses.create`)를 전송해야 성공적인 답변 메커니즘을 이끌어낼 수 있습니다.

---

**8장에서의 인증 획득 구성 및 우선순위**

- **Microsoft Entra ID 토큰 방식 (8.2, 8.3 필수)**: 로컬 터미널에서 **`az login`**을 맨 먼저 실행해 주거나 VS Code Azure Account에 로그인해 로그 정보를 활성화해 둡니다. (만약 기업용 네트워크 가드 하에 있다면 `az config set core.no_ssl_verify=true`를 전역으로 적용해 검증을 생략해야 합니다)
- **API 키 (8.1 대화 구동에만 권장)**: `.env`에 `FOUNDRY_API_KEY`가 설정되어 있으면 일반 8.1 실행에서는 가벼운 Bearer 토큰 연계로 활용됩니다.

> **8.2 주의**: `ensure_kb_mcp_project_connection`이 Knowledge base MCP project connection을 ARM REST API(`management.azure.com`)로 생성할 때는 Azure AD 토큰이 필요합니다. connection이 이미 포털에 존재하면 이 단계가 실패해도 agent 실행에는 문제 없습니다.
...
5. 프로젝트에 `gpt-5.2` 배포가 있는지 확인합니다.
6. 다른 배포 이름을 쓰는 경우 `.env`의 `FOUNDRY_MODEL_DEPLOYMENT_NAME`을 실제 배포 이름으로 바꿉니다.
7. `.env`에 `FOUNDRY_API_KEY`가 이미 설정되어 있으면 이 단계는 건너뜌어도 됩니다. 없으면 로컈 터미널에서 `az login`을 실행하거나(Windows는 PowerShell, macOS/Linux는 zsh, bash) VS Code Azure Account에 로그인합니다.
...
### 8.1용 Foundry Search connection 준비
...
### 8.4용 Application Insights 수동 생성
...
공통 값은 다음과 같습니다. `FOUNDRY_API_KEY`가 설정되어 있으면 `az login` 없이 8.1과 8.3을 실행할 수 있습니다.
...
## 8.4 Application Insights와 Monitoring
...
## 8.1 직접 Search tool과 8.2 Knowledge base MCPTool의 차이
...
## 6장 로컬 MCP 스타일 서버와 8장 Foundry MCP tool의 차이
...
- `.env`에 `FOUNDRY_API_KEY`가 설정되어 있거나, 터미널에서 `az login`을 완료했거나(Windows는 PowerShell, macOS/Linux는 zsh, bash) VS Code Azure Account에 로그인되어 있습니다.
