...
회사 네트워크에서 Zscaler 같은 TLS inspection/proxy를 사용하는 경우 `az login` 또는 Python SDK 호출이 `CERTIFICATE_VERIFY_FAILED`로 실패할 수 있습니다. 이때는 Zscaler Root CA를 Windows, Azure CLI, Python이 신뢰하도록 등록하거나 실습 중 프록시를 비활성화한 뒤 `az login`을 먼저 확인합니다.
...
7. 로컬 PowerShell에서 `az login`을 실행하거나 VS Code Azure Account에 로그인합니다.
...
- PowerShell에서 `az login`을 완료했거나 VS Code Azure Account에 로그인되어 있습니다.
